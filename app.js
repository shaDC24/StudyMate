...
app.get("/student/update/:taskId", async (req, res) => {
    try {
        ...
        //console.log(task);
        if (task) {
            res.render("update_routine", { task, errorMessage: false, successMessage: false, warningmessage: false });
        } else {
...
app.post("/student/update/:taskId", async (req, res) => {
    try {
        ...
        console.log(taskId);
        const current_task = await db.one('select *from routine where id=$1', taskId);
        if (current_task.task_name === taskName1 && current_task.start_time === startTime1 && current_task.end_time === endTime1) {
            return res.render("update_routine", {
                ...
                errorMessage: false, successMessage: false, warningmessage: 'Same as previous no updation'
            });
        }
        ...
        if (existingTask) {
            ...
            return res.render("update_routine", {
                ...
                errorMessage, successMessage: false, warningmessage: false
            });
        }
        ...
        return res.render("update_routine", {
            ...
            errorMessage: false, successMessage: 'successfully updated', warningmessage: false
        });
    } catch (error) {
        console.error("Error updating task:", error);
        res.status(500).json({ error: "Error updating task." });
